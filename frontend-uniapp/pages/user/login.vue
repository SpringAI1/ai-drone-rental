<template>
  <view class="login-page">
    ...
    <view class="hero-section">
      ...
      <image class="hero-image" src="/static/drones/mavic3_drone.png" mode="aspectFit" />
      <view class="hero-content">
        <text class="hero-title">AI 无人机租赁</text>
        <text class="hero-subtitle">✦ 开启您的飞行之旅 ✦</text>
      </view>
    </view>
    ...
    <view class="login-card">
      <view class="card-title-box">
        ...
        <text class="card-subtitle">登录您的账号继续飞行</text>
      </view>
      ...
      <view class="submit-btn" :class="{ loading: loading }" @click="handleLogin">
        ...
        <text class="submit-text">{{ loading ? '登录中...' : '立即登录' }}</text>
      </view>
      ...
      <!-- 注册链接 -->
      <view class="register-link-box">
        <text class="link-ask">还没有账号？</text>
...
const handleLogin = async () => {
  ...
  loading.value = true
  try {
    // 1. 登录后端接口 /auth/login → 返回 LoginVO{userId,username,nickname,role,token}
    const loginRes = await login({ username: username.value, password: password.value })
    const loginData: any = (loginRes.data as any) || {}
    const token = loginData.token || ''
    const basicUser = {
      id: Number(loginData.userId || loginData.id || 0),
      username: loginData.username || username.value,
      nickname: loginData.nickname || loginData.username,
      role: loginData.role !== undefined ? Number(loginData.role) : 0
    }

    // 先把 token 存起来，这样后续请求就带上了 Authorization: Bearer xxx
    auth.setToken(token)
    auth.login(token, basicUser as any)

    // 2. 用 token 调用 /user/info 获取真实用户信息（phone、email、avatar、balance 等）
    try {
      const infoRes = await getUserInfo()
      if (infoRes.data) {
        auth.updateUserInfo(infoRes.data)
      }
    } catch (err2) {
      console.warn('获取用户信息失败，但不影响登录', err2)
    }
...
.hero-bg {
  ...
  right: 0;
  bottom: 0;
  background: linear-gradient(180deg, #1e3a8a 0%, #2563eb 40%, #3b82f6 70%, #60a5fa 100%);
}
...
.login-card {
  ...
  box-shadow: 0 -20rpx 60rpx rgba(30, 58, 138, 0.15),
              0 20rpx 60rpx rgba(37, 99, 235, 0.08);
  position: relative;
  z-index: 2;
}
