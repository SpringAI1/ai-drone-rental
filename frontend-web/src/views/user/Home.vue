...
const stats = ref([
  { value: '--', label: '设备型号', icon: markRaw(Goods), color: 'primary' },
  { value: '--', label: '累计订单', icon: markRaw(TrendCharts), color: 'success' },
  { value: '--', label: '注册用户', icon: markRaw(User), color: 'info' },
  { value: '--', label: '好评率', icon: markRaw(Medal), color: 'warning' }
])
...
const fetchStats = async () => {
  try {
    ...
    const data = res.data || {}
    stats.value = [
      { value: `${data.totalDrones || 0}+`, label: '设备型号', icon: markRaw(Goods), color: 'primary' },
      { value: `${data.totalOrders || 0}+`, label: '累计订单', icon: markRaw(TrendCharts), color: 'success' },
      { value: `${data.totalUsers || 0}+`, label: '注册用户', icon: markRaw(User), color: 'info' },
      { value: `${data.positiveRate || 0}%`, label: '好评率', icon: markRaw(Medal), color: 'warning' }
    ]
  } catch (error) {
