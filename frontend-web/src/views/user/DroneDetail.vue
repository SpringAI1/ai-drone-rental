          ...
          <div v-if="comments.length > 0" class="comment-list">
            <div v-for="comment in comments" :key="comment.id" class="comment-item">
              ...
              <div class="comment-actions">
                <el-button text size="small" @click="showReplyDialog(comment)">回复</el-button>
              </div>
              ...
              <div v-if="comment.children && comment.children.length > 0" class="child-comments">
                <div v-for="child in comment.children" :key="child.id" class="child-comment-item">
                  <div class="child-comment-header">
                    <el-avatar :size="28" :src="getAvatarUrl(child.userAvatar)">
                      {{ child.userNickname?.charAt(0) || 'U' }}
                    </el-avatar>
                    <span class="child-comment-user">{{ child.userNickname || '匿名用户' }}</span>
                    <span class="child-comment-time">{{ formatCommentTime(child.createTime) }}</span>
                  </div>
...
const imageList = computed(() => {
  if (!drone.value?.image) return ['https://picsum.photos/600/400']
